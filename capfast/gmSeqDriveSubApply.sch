[schematic2]
uniq 130
[tools]
[detail]
w 2372 1139 100 0 n#128 estringouts.errMess.SLNK 2432 1520 2368 1520 2368 768 2496 768 2496 624 2352 624 efanouts.fan2.LNK1
w 2506 603 100 0 n#127 efanouts.fan2.LNK2 2352 592 2720 592 2720 672 2880 672 elongouts.getSubapplyC.SLNK
w 4274 587 100 0 n#125 efanouts.fanStartTimer.LNK2 4080 592 4160 592 4160 576 4448 576 eaos.startApplyCTrig.SLNK
w 3754 555 100 0 n#123 ecalcs.calcApplyBusy.FLNK 3696 624 3728 624 3728 544 3840 544 efanouts.fanStartTimer.SLNK
w 3794 635 100 0 n#122 ecalcs.calcApplyBusy.VAL 3696 592 3760 592 3760 624 3888 624 efanouts.fanStartTimer.SELL
w 3298 795 100 0 n#120 elongouts.getSubapplyC.VAL 3136 672 3248 672 3248 784 3408 784 ecalcs.calcApplyBusy.INPA
w 3316 547 100 0 n#119 elongouts.getSubapplyC.FLNK 3136 704 3312 704 3312 400 3408 400 ecalcs.calcApplyBusy.SLNK
w 90 1675 100 0 DISV inhier.SDIS.P 32 1664 208 1664 208 1744 368 1744 estringouts.null.SDIS
w 1378 1979 100 0 n#110 efanouts.fan1.FLNK 1328 1968 1488 1968 1488 2048 1600 2048 gmSeqTimeOut.gmSeqTimeOut#17.START
w 1898 1467 100 0 n#107 estringouts.subApplyMess.FLNK 2304 864 2336 864 2336 1456 1520 1456 1520 1856 1600 1856 gmSeqTimeOut.gmSeqTimeOut#17.STOP
w 1056 1867 100 0 n#95 efanouts.fan1.SLNK 1088 1856 1072 1856 1072 1264 1056 1264 elongouts.subsysDir.FLNK
w 728 1243 100 0 n#93 elongouts.busy.FLNK 624 1536 704 1536 704 1232 800 1232 elongouts.subsysDir.SLNK
w 640 1275 100 0 DIR inhier.DIR.P 528 1264 800 1264 elongouts.subsysDir.DOL
w 1824 523 100 0 n#91 ecalcs.subApplyValCalc.VAL 1744 512 1952 512 1952 624 2160 624 efanouts.fan2.SELL
w 1900 211 100 0 SUBVAL junction 1280 464 1280 208 2592 208 outhier.SUBVAL.p
w 1352 715 100 0 SUBVAL eais.subApplyVal.VAL 1264 464 1280 464 1280 704 1456 704 ecalcs.subApplyValCalc.INPA
w 824 603 100 0 n#89 ecalcouts.wait.VAL 704 592 992 592 992 512 1008 512 eais.subApplyVal.INP
w 1288 507 100 0 n#88 eais.subApplyVal.FLNK 1264 496 1360 496 1360 320 1456 320 ecalcs.subApplyValCalc.SLNK
w 808 667 100 0 n#87 ecalcouts.wait.FLNK 704 656 960 656 960 480 1008 480 eais.subApplyVal.SLNK
w 2052 875 100 2 c#82 lboat.c#82.p 2048 880 2048 880 estringouts.subApplyMess.DOL
w 2136 731 100 0 n#80 efanouts.fan2.FLNK 2352 656 2400 656 2400 720 1920 720 1920 848 2048 848 estringouts.subApplyMess.SLNK
w 2692 1499 100 2 c#75 rboat.c#75.p 2688 1504 2688 1504 estringouts.errMess.OUT
w 2692 2027 100 2 c#74 rboat.c#74.p 2688 2032 2688 2032 estringouts.noResp.OUT
w 2692 1755 100 2 c#73 rboat.c#73.p 2688 1760 2688 1760 eaos.errVal.OUT
w 224 667 100 0 c#71 lboat.c#71.p 112 704 112 656 384 656 ecalcouts.wait.INPA
w 2882 1608 100 0 n#57 eaos.errVal.FLNK 2688 1824 2880 1824 2880 1344 2560 1344 2560 1168 2624 1168 eaos.stopApplyCTrig.SLNK
w 2576 1202 100 0 n#54 hwin.hwin#56.in 2576 1200 2624 1200 eaos.stopApplyCTrig.DOL
w 2552 1635 100 0 n#53 estringouts.errMess.FLNK 2688 1536 2752 1536 2752 1632 2400 1632 2400 1792 junction
w 2552 1923 100 0 n#53 estringouts.noResp.FLNK 2688 2064 2752 2064 2752 1920 2400 1920 2400 1792 2432 1792 eaos.errVal.SLNK
w 1904 555 100 0 n#49 ecalcs.subApplyValCalc.FLNK 1744 544 2112 544 efanouts.fan2.SLNK
w 2216 2051 100 0 n#40 gmSeqTimeOut.gmSeqTimeOut#17.EXPIRED 2048 2048 2432 2048 estringouts.noResp.SLNK
w 4400 611 100 0 n#38 hwin.hwin#21.in 4400 608 4448 608 eaos.startApplyCTrig.DOL
w 2376 1826 100 0 n#29 hwin.hwin#28.in 2368 1824 2432 1824 eaos.errVal.DOL
w 472 1634 100 0 n#14 estringouts.null.FLNK 624 1792 720 1792 720 1632 272 1632 272 1504 368 1504 elongouts.busy.SLNK
w 102 1810 100 0 n#7 inhier.SLNK.P 32 1808 208 1808 208 1776 368 1776 estringouts.null.SLNK
w 294 1538 100 0 n#5 hwin.hwin#4.in 256 1536 368 1536 elongouts.busy.DOL
s 1408 128 130 0 (Command not rejected)
s 288 192 130 0 Wait for monitor on apply.VAL
s 3344 240 100 0 output 1 if BUSY, 2 if not
s 4160 512 100 0 LNK2 if applyC not BUSY
s 3376 272 100 0 Test for CAR_BUSY (2)
s 2432 560 100 0 LNK2 if apply.VAL positive
s 2448 336 130 0 Trigger timer only if apply.VAL positive AND applyC not BUSY
s 1360 176 140 0 Check for positive apply.VAL
s 1904 944 100 0 top-level apply record
s 1904 976 100 0 This string is input to the
s 4480 2976 150 0 gmSeqDriveSubApply
s 1568 1376 140 0 See schematic gmSeqCarMonitor
s 1568 1328 140 0 for CommSentC, StartapplyCTimer and
s 1568 1280 140 0 StopapplyCTimer records
s 2688 816 100 0 Get current applyC value from subsystem
s 3440 832 100 0 Check whether applyC is BUSY
s 4160 768 140 0 Trigger timeout start if not BUSY
s 2432 640 100 0 LNK1: subsys error
[cell use]
use ecalcouts 384 471 100 0 wait
xform 0 544 592
p 456 504 100 0 -1 CALC:A
p 984 1292 100 0 0 DESC:Wait for subsystem Apply accept/reject
p 1216 1022 100 0 0 DOPT:Use CALC
p 1216 1054 100 0 0 OOPT:Every Time
p 472 704 100 0 1 SCAN:I/O Intr
p 800 848 60 0 0 def(INPA):$(prefix)apply.VAL
use elongouts 824 1144 100 0 subsysDir
xform 0 928 1232
p 832 1326 100 0 -1 DESC:Set directive in subsystem APPLY
p 864 1104 100 0 1 OMSL:closed_loop
p 1168 1198 100 0 -1 def(OUT):$(prefix)apply.DIR
p 1056 1200 75 768 -1 pproc(OUT):PP
use elongouts 392 1416 100 0 busy
xform 0 496 1504
p 400 1598 100 0 -1 DESC:Output BUSY value
p 432 1376 100 0 1 OMSL:closed_loop
p 736 1470 100 0 -1 def(OUT):$(gm)$(subsys)CommSentC.IVAL
p 624 1472 75 768 -1 pproc(OUT):PP
use elongouts 2880 583 100 0 getSubapplyC
xform 0 3008 672
p 2928 752 100 0 1 OMSL:closed_loop
p 2528 736 100 0 1 def(DOL):$(prefix)applyC.IVAL
use efanouts 2136 408 100 0 fan2
xform 0 2232 560
p 2096 672 100 0 1 SELM:Specified
use efanouts 1136 1808 100 0 fan1
xform 0 1208 1872
p 1056 1984 100 0 1 SELM:All
use efanouts 3840 407 100 0 fanStartTimer
xform 0 3960 560
p 3824 672 100 0 1 SELM:Specified
use ecalcs 1480 232 100 0 subApplyValCalc
xform 0 1600 496
p 1536 446 100 0 1 CALC:A<0?1:2
use ecalcs 3408 311 100 0 calcApplyBusy
xform 0 3552 576
p 3472 640 100 0 1 CALC:A=2?1:2
use eaos 4472 488 100 0 startApplyCTrig
xform 0 4576 576
p 4496 670 100 0 -1 DESC:Start timer on subsystem applyC
p 4512 432 100 0 1 OMSL:closed_loop
p 4432 368 100 0 1 def(OUT):$(gm)$(subsys)StartapplyCTimer.PROC
p 4704 544 75 768 -1 pproc(OUT):PP
use eaos 2456 1704 100 0 errVal
xform 0 2560 1792
p 2464 1870 100 0 -1 DESC:Write error status
p 2496 1664 100 0 1 OMSL:closed_loop
p 2720 1790 100 0 -1 def(OUT):$(gm)$(subsys)CommSentC.IVAL
p 2688 1760 75 768 -1 pproc(OUT):PP
use eaos 2648 1080 100 0 stopApplyCTrig
xform 0 2752 1168
p 2672 1262 100 0 -1 DESC:Cancel timer on subsystem applyC
p 2688 1024 100 0 1 OMSL:closed_loop
p 2528 1054 100 0 1 def(OUT):$(gm)$(subsys)StopapplyCTimer.PROC
p 2880 1136 75 768 -1 pproc(OUT):PP
use oslBorderD -208 -265 100 0 oslBorderD#114
xform 0 2432 1440
use inhier 448 1248 150 0 DIR
xform 0 528 1264
use inhier 40 1768 100 0 SLNK
xform 0 32 1808
use inhier 16 1623 100 0 SDIS
xform 0 32 1664
use rboat 2688 1463 100 0 c#75
xform 0 2768 1504
use rboat 2688 1991 100 0 c#74
xform 0 2768 2032
use rboat 2688 1719 100 0 c#73
xform 0 2768 1760
use hwin 88 1496 100 0 hwin#4
xform 0 160 1536
p 67 1528 100 0 -1 val(in):$(CAR_BUSY)
use hwin 4232 568 100 0 hwin#21
xform 0 4304 608
p 4211 600 100 0 -1 val(in):1
use hwin 2200 1784 100 0 hwin#28
xform 0 2272 1824
p 2179 1816 100 0 -1 val(in):$(CAR_ERROR)
use hwin 2408 1160 100 0 hwin#56
xform 0 2480 1200
p 2387 1192 100 0 -1 val(in):1
use eais 1008 407 100 0 subApplyVal
xform 0 1136 480
use outhier 2560 167 100 0 SUBVAL
xform 0 2576 208
use lboat -48 663 100 0 c#71
xform 0 32 704
use lboat 1888 839 100 0 c#82
xform 0 1968 880
use estringouts 392 1704 100 0 null
xform 0 496 1776
p 432 1854 100 0 -1 DESC:Empty string
p 432 1664 100 0 1 OMSL:closed_loop
p 304 1742 100 0 0 VAL:
p 752 1758 100 0 -1 def(OUT):$(gm)$(subsys)CommSentC.IMSS
p 16 1744 100 0 0 def(SDIS):0.0
use estringouts 2456 1976 100 0 noResp
xform 0 2560 2048
p 2448 2126 100 0 -1 DESC:No response string
p 2496 1936 100 0 1 OMSL:closed_loop
p 2368 2014 100 0 0 VAL:No response from $(instru)
p 2768 2062 100 0 -1 def(OUT):$(gm)$(subsys)CommSentC.IMSS
use estringouts 2456 1448 100 0 errMess
xform 0 2560 1520
p 2384 1424 100 0 -1 DESC:Subsys error message
p 2496 1392 100 0 1 OMSL:closed_loop
p 2368 1486 100 0 0 VAL:
p 2224 1584 100 0 -1 def(DOL):$(prefix)apply.MESS
p 2752 1534 100 0 -1 def(OUT):$(gm)$(subsys)CommSentC.IMSS
use estringouts 2048 775 100 0 subApplyMess
xform 0 2176 848
p 2112 736 100 0 1 OMSL:closed_loop
p 1792 912 100 0 -1 def(DOL):$(prefix)apply.MESS
use gmSeqTimeOut 1624 1800 100 0 gmSeqTimeOut#17
xform 0 1824 1952
p 1824 1854 100 0 -1 seta:timeout 3.0
[comments]
