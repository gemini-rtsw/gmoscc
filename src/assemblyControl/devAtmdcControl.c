...
)
{
	 ...
	 */
        ...
        CHECKSTAT((status = dbPutLink(velLink, DBR_DOUBLE, &(pDevAss->velocity), nRequest)),return(status));
	...
	status = dbPutLink(ackLink, DBR_USHORT, &returnedAck, nRequest);
...
/* Is a command active? If not, do nothing */
    if (pDevAss->cmdActive)
     {
/* Finish the command */
       DEBUG(DAR_DBUG_MIN, "<%ld> %s:atmProcessFault: interlock occurred%c\n",' ');
