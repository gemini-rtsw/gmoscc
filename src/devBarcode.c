...
#define ARM             "\x23"  /* arming character to activate switch */
...
)
{
    ...
    {
       DEBUG(BAR_DBUG_NONE,"Barcode init_chan: Invalid barcode channel number: %d\n", chan);
       return(ERROR);
    }
    ...
    if (!devBarcodePortInitialized)
    {
       ...
       DEBUGBAR(BAR_DBUG_FULL,"Initializing barcode device:%s chan:%d\n",chan);
       ...
       if (!Qin)
       {
          DEBUGBAR(BAR_DBUG_NONE,
                   "Barcode d:%s c:%d Can't create input queue\n",chan);
          return(ERROR);
       }
       ...
       {
          DEBUGBAR(BAR_DBUG_NONE,
                   "Barcode d:%s c:%d Can't create output queue\n",chan);
          return(ERROR);
       }
       ...
       DEBUGBAR(BAR_DBUG_FULL,
                "Barcode d:%s c:%d Spawning monitor_port...\n",chan);
       ...
       {
          DEBUGBAR(BAR_DBUG_NONE,
                   "Barcode d:%s c:%d Can't create monitor_port task\n",chan);
          return(ERROR);
       }
 
       DEBUGBAR(BAR_DBUG_FULL,
                "Barcode d:%s c:%d Spawning return_scan...\n",chan);
       ...
       DEBUGBAR(BAR_DBUG_FULL,
                "Barcode d:%s c:%d Setting port parameters\n",chan);
       ...
       if (!devBarcodeSimulate)
       {
          ...
          {
             DEBUGBAR(BAR_DBUG_NONE,
                      "Barcode d:%s c:%d Serial port open fail\n",chan);
             return(ERROR);
          }
          ...
          {
             DEBUGBAR(BAR_DBUG_NONE,
                      "Barcode d:%s c:%d Can't configure port.\n",chan);
             close (scanfd);
             return(ERROR);
          }
    ...
    {
       DEBUGBAR(BAR_DBUG_FULL,
                "Barcode d:%s c:%d already initialized.\n",chan);
    }
    ...
    {
       DEBUGBAR(BAR_DBUG_NONE,
                "Barcode d:%s c:%d  Duplicate channel.\n",chan);
       return(ERROR);
    }
    ...
    {
       DEBUGBAR(BAR_DBUG_NONE,
                "Barcode d:%s c:%d This device not configured\n",chan);
    ...
    DEBUGBAR(BAR_DBUG_FULL,
             "Barcode device:%s  Setting up Scanner on channel %d...\n",chan);
    ...
    {
       DEBUGBAR(BAR_DBUG_NONE,
                "Barcode d:%s c:%d Can't open serial port.\n",chan);
       return(ERROR);
    }
    ...
    {
       DEBUGBAR(BAR_DBUG_NONE,
                "Barcode d:%s c:%d Can't connect to switch.\n",chan);
       close(scanfd);
       return(ERROR);
    }
    ...
    {
       DEBUGBAR(BAR_DBUG_NONE,
                "Barcode d:%s c:%d SET_DEFAULTS failed.\n",chan);
    ...
    {
       DEBUGBAR(BAR_DBUG_NONE,
                "Barcode d:%s c:%d INIT_PARM failed.\n",chan);
       close_switch(scanfd,chan);
       close(scanfd);
       return (ERROR);
    }
...
)    
{
    ...
    {
       DEBUG(BAR_DBUG_NONE,
             "devBarcodeConfig: Bad channel number, %d\n",chan);
       return(ERROR);
    }
    ...
    {
       DEBUG(BAR_DBUG_NONE,
             "devBarcodeConfig: Cannot connect to switch channel %d.\n",chan);
       close(scanfd);
       return(ERROR);
    }
    ...
    DEBUG(BAR_DBUG_MIN,
          "Setting up barcode scanner on channel %d... \n", chan);
    DEBUG(BAR_DBUG_MAX,
...
)
{
    ...
    if ((chan < 0)||(chan >= NUM_CHAN))
    {
       DEBUG(BAR_DBUG_NONE,"Barcode trigger_scan: Invalid barcode channel number: %d\n", chan);
       return(ERROR);
    }
    ...
    {
       DEBUGBAR(BAR_DBUG_FULL,
                "Barcode device:%s chan:%d Finishing processing.\n",chan);
       return(0);
    }
    ...
    sprintf(message,"%d",chan);
    DEBUGBAR(BAR_DBUG_FULL,
             "Barcode device:%s Placing channel %d into outgoing Q.\n",chan);

    status = msgQSend(Qout, message, sizeof(message), 10, MSG_PRI_NORMAL);
    if (status != 0)
    {
       DEBUGBAR(BAR_DBUG_NONE,
                "Barcode device:%s Message Qout write for %d failed.\n",chan);
    }
...
static void monitor_port ()
{
    ...
    for (;;)
    {
        ...
        */
        ...
        {
            DEBUG(BAR_DBUG_NONE,
                  "Barcode monitor_port: Invalid channel number from Qout, %d\n",chan);
            error = 1;
        }
        ...
        else if (devBarcodeSimulate)
        {
            ...
            {
                DEBUGBAR(BAR_DBUG_FULL,
                         "Barcode device:%s chan:%d Faking barcode....\n",chan);
                sprintf(pbar[chan].barcode,"%8d", BAR_SIM_BASE + devBarcodeSimValue);
                devBarcodeSimValue++;
            ...
            {
                DEBUGBAR(BAR_DBUG_FULL,
                         "Barcode device:%s chan:%d Faking barcode NR....\n",chan);
                strcpy(pbar[chan].barcode, "NR");
            }
            else
            {
                DEBUGBAR(BAR_DBUG_FULL,
                         "Barcode device:%s chan:%d Faking barcode ERROR....\n",chan);
        ...
        else if (!error)
        {
            ...
            {
                DEBUGBAR(BAR_DBUG_NONE,
                         "Barcode d:%s c:%d Open port failed.\n",chan);
            ...
            {
                DEBUGBAR(BAR_DBUG_NONE,
                         "Barcode device:%s chan:%d Switch failed.\n",chan);
                close (scanfd);
            ...
            for (rescan = 0; !error && rescan < MAX_RESCANS; rescan++)
            {
                ...
                {
                    DEBUGBAR(BAR_DBUG_NONE,
                         "Barcode d:%s c:%d No ack on write.\n",chan);
                    close_switch(scanfd,chan);
                    close (scanfd);   
                ...
                if (!error)
                {
                    DEBUGBAR(BAR_DBUG_MIN,
                             "Barcode d:%s c:%d Waiting...\n",chan);
     
                    if (ReadScanner(scanfd, rpy_string) == ERROR)
                    {
                        DEBUGBAR(BAR_DBUG_NONE,
                                 "Barcode d:%s c:%d Read failed, retry\n",
                                 chan);
        ...
        {
             DEBUG(BAR_DBUG_NONE,
                "Barcode monitor_port: Message Qin write for channel %d failed.\n",chan);
        }
        error = 0;
    }
}
...
static void return_scan()
{
    ...
    for (;;)
    {
       ...
       if ((chan < 0)||(chan >= NUM_CHAN))
       {
          ...
          DEBUG(BAR_DBUG_NONE,
                "Barcode return_scan: Invalid channel from Qin, %d.\n",chan);
       }
       else
       {
...
)
{
    ...
    DEBUGBAR(BAR_DBUG_FULL,
         "Barcode device:%s chan:%d Opening switchbox connection.\n",chan);

    strncpy(cmd_string, (char *) ARM, MAX_STRING-1);
    ...
    {
       DEBUGBAR(BAR_DBUG_NONE,
                "Barcode d:%s c:%d Failed to establish switch connection.\n",
                chan);
       return (ERROR);
    }
    return (0);
...
)
{
    ...
    DEBUGBAR(BAR_DBUG_FULL,
             "Barcode d:%s c:%d Closing switchboxconnection.\n",chan);
    ...
    {
       DEBUGBAR(BAR_DBUG_NONE,
                "Barcode d:%s c:%d Failed to closeswitch box connection.\n",
                chan);
       return (ERROR);
    }
    return (0);
