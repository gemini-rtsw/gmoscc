...
)
{
    ...
    if ( par->busy == DAR_BUSY_BUSY)
    {
     ...
     if ( par->mode == DAR_MODE_MOVE )
     {
         ...
         else
         {
              ...
              {
                  if (sscanf (par->a, "%d%s", &barcodeIdA, dumpString) != 1)
                  {
                      status = DAR_E_ATT;
              ...
              {
                  if (sscanf (par->b, "%d%s", &barcodeIdB, dumpString) != 1)
                  {
     ...
     else if ( par->mode == DAR_MODE_PARK )
     {
         ...
         {
              FLDEBUG(DAR_MSG_ERROR, "flCheckAttributes: Park position %d out of range\n",
                  *(long *)par->c);
     ...
     else if ( par->mode == DAR_MODE_UPDATE )
     {
         ...
         {
             FLDEBUG(DAR_MSG_ERROR, "flCheckAttributes: UPDATE mode (%d) must be 0 or 1\n",
                *(long *)par->d);
	 ...
	 *(long *)par->d = 0;
     ...
     if ( status == DAR_S_SUCCESS )
     {
          ...
          {
              FLDEBUG(DAR_MSG_ERROR, "flCheckAttributes: Switches flag (%d) must be 0 or 1\n",
                 *(long *)par->e);
         ...
         case 1:
              if ( strncmp( FILT1_BARCODE_READ_FIELD, "NR", 2 ) != 0 )
              {
                           ...
                           )
                   {
                       ...
                       FLDEBUG(DAR_MSG_WARNING,
                          "flCheckBarcodeId: Barcode for wheel 1 at position %d out of range\n",
                          lutIndex );
                       semTake (pFlPriv->mutexSem, WAIT_FOREVER);
                       pDevConfig->badBarcode = TRUE;
                       semGive (pFlPriv->mutexSem);
              ...
              else
              {
                   ...
                   FLDEBUG(DAR_MSG_WARNING,
                      "flCheckBarcodeId: Unreadable barcode for wheel 1 at position %d\n",
                      lutIndex );
                   semTake (pFlPriv->mutexSem, WAIT_FOREVER);
                   pDevConfig->badBarcode = TRUE;
                   semGive (pFlPriv->mutexSem);
         ...
         case 2:
              if ( strncmp( FILT2_BARCODE_READ_FIELD, "NR", 2 ) != 0 )
              {
                           ...
                           )
                   {
                       ...
                       FLDEBUG(DAR_MSG_WARNING,
                          "flCheckBarcodeId: Barcode for wheel 2 at position %d out of range\n",
                          lutIndex );
                       semTake (pFlPriv->mutexSem, WAIT_FOREVER);
                       pDevConfig->badBarcode = TRUE;
                       semGive (pFlPriv->mutexSem);
              ...
              else
              {
                   ...
                   FLDEBUG(DAR_MSG_WARNING,
                      "flCheckBarcodeId: Unreadable barcode for wheel 2 at position %d\n",
                      lutIndex );
                   semTake (pFlPriv->mutexSem, WAIT_FOREVER);
                   pDevConfig->badBarcode = TRUE;
                   semGive (pFlPriv->mutexSem);
     ...
     else
     {
          ...
          while (fgets(scratchBuf, sizeof scratchBuf, fp) != NULL) 
          {
               ...
               if (*scratchBuf != '#' && *scratchBuf != '\n')     /* a comment or empty line, discard it */
               {
                    ...
                    if (sscanf(scratchBuf, "%d %d %s", &barcode, &wheel, pos) != 3)
                    {
