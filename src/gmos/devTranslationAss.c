...
/*	CHECKSTAT( (status = recGblPutLinkValue( posLink, (void *) par, 
	...
	CHECKSTAT( (status = dbPutLink(posLink, DBR_STRING, svals,nRequest)), return(status));
	...
	CHECKSTAT( (status = dbPutLink(velLink, DBR_DOUBLE, &(pDtaPriv->velocity[i]),nRequest)), return(status));
	...
	status = dbGetLink(ackLink, DBR_USHORT, &returnedAck ,0,0);
...
/* COLDTESTS - the following line can be removed after thermal model is finalized  */
	  ...
	  */
	  dbPutLink(&(par->sou),DBR_DOUBLE,&(temperature),one);
          dbPutLink(&(par->sov),DBR_DOUBLE,&(filteredTemperature),one);
...
/* COLDTESTS - the following two lines can be removed after thermal model is finalized  */
	  ...
	  */
	  dbPutLink(&(par->sor),DBR_DOUBLE,&(xtalkDeltaX),one);
          dbPutLink(&(par->sos),DBR_DOUBLE,&(xtalkDeltaY),one);
...
/* COLDTESTS - the following two lines can be removed after the GMOS thermal model is finalized  */
    ...
    */
     dbPutLink(&(par->sot),DBR_DOUBLE,&(pDtaConfig->zeroOffsetTemperature),one); 
     /*
