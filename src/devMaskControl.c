...
)
{
    ...
    DEBUG(DAR_MSG_MAX, "<%d> %s:mkAckReceived: entry, cmdActive=%d\n", 
            pMkPriv->cmdActive );
    ...
    {
        DEBUG(DAR_MSG_FULL, "<%d> %s:mkAckReceived:unexpected cmd change%c\n",
              ' ');
    }
    ...
    {
        DEBUG(DAR_MSG_FULL, "<%d> %s:mkAckReceived:barcode read timeout%c\n", 
              ' ');
    ...
    {
        DEBUG(DAR_MSG_FULL, "<%d> %s:mkAckReceived:optoDelayOn timeout%c\n", 
              ' ');
        status = mkTaskFinished( par );
    }
    ...
    else if ( assCommandTimedOut( par ) )
    {
        ...
        DEBUG(DAR_MSG_ERROR, "<%d> %s:mkAckReceived: command timeout%c\n", ' ');
    ...
    else if ( bus1 || bus2 || bus3 || bus4 )
    {
        ...
        if (bus1) switch (par->bus1)
        {
            case DAR_DEV_BUSY_IDLE:
                DEBUG(DAR_MSG_FULL, "<%d> %s:mkAckReceived:Ext finished%c\n",
                      ' ');
            ...
            case DAR_DEV_BUSY_BUSY:
                DEBUG(DAR_MSG_FULL, "<%d> %s:mkAckReceived:Ext started%c\n",
                    ' ');
                break;
            ...
            case DAR_DEV_BUSY_ERROR:
                DEBUG(DAR_MSG_ERROR, "<%d> %s:mkAckReceived: Ext failed%c\n",
                      ' ');
        ...
        if (!status && bus2) switch (par->bus2)
        {
            ...
            case DAR_DEV_BUSY_IDLE:
                DEBUG(DAR_MSG_FULL, "<%d> %s:mkAckReceived:Grp finished%c\n", 
                      ' ');
                semTake (pMkPriv->mutexSem, WAIT_FOREVER);
            ...
            case DAR_DEV_BUSY_BUSY:
                DEBUG(DAR_MSG_FULL, "<%d> %s:mkAckReceived: Grp started%c\n",
                  ' ');
                break;
            ...
            case DAR_DEV_BUSY_ERROR:
                DEBUG(DAR_MSG_ERROR, "<%d> %s:mkAckReceived: Grp failed%c\n",
                      ' ');
        ...
        if (!status && bus3) switch (par->bus3)
        {
            ...
            case DAR_DEV_BUSY_IDLE:
                DEBUG(DAR_MSG_FULL, "<%d> %s:mkAckReceived: Cas finished%c\n",
                      ' ');
                semTake (pMkPriv->mutexSem, WAIT_FOREVER);
            ...
            case DAR_DEV_BUSY_BUSY:
                DEBUG(DAR_MSG_FULL, "<%d> %s:mkAckReceived: Cas started%c\n",
                      ' ');
                break;
            ...
            case DAR_DEV_BUSY_ERROR:
                DEBUG(DAR_MSG_ERROR, "<%d> %s:mkAckReceived: Cas failed%c\n",
                      ' ');
        ...
        if (!status && bus4) switch (par->bus4)
        {
            ...
            case DAR_DEV_BUSY_IDLE:
                DEBUG(DAR_MSG_FULL, "<%d> %s:mkAckReceived: Rel finished%c\n", 
                      ' ');
                semTake (pMkPriv->mutexSem, WAIT_FOREVER);
            ...
            case DAR_DEV_BUSY_BUSY:
                DEBUG(DAR_MSG_FULL, "<%d> %s:mkAckReceived: Rel started%c\n",
                  ' ');
                break;
            ...
            case DAR_DEV_BUSY_ERROR:
                DEBUG(DAR_MSG_ERROR, "<%d> %s:mkAckReceived: Release failed%c\n",
                      ' ');
    ...
    {
        DEBUG(DAR_MSG_ERROR, "<%d> %s:mkAckReceived, invalid bus response %c\n",
              ' ');
    } 
...
)
{
    ...
    DEBUG(DAR_MSG_FULL, "<%d> %s:mkBuildList: entry%c\n", ' ');
    ...
    {
        DEBUG(DAR_MSG_ERROR, "<%d> %s:mkBuildList: taskList not empty =%d\n", 
    ...
    DEBUG(DAR_MSG_FULL, 
          "<%d> %s:mkBuildList: put opto sensor power up on list%c\n", ' ');
...
)
{
    ...
    DEBUG(DAR_MSG_FULL, "<%d> %s:mkBuildNewList: entry%c\n", ' ');
    ...
    switch( mode )
    {
        ...
        {
            DEBUG(DAR_MSG_FULL, 
                  "<%d> %s:mkBuildNewList: Ext not grip/release area%c\n", ' ');
        }
        else
        {
            DEBUG(DAR_MSG_FULL, 
                  "<%d> %s:mkBuildNewList: Ext in grip/release area%c\n", ' ');
        }

        if ( !( MK_SENSOR_WORD & MK_MON_NOTINSTALLED ) )
        {
            DEBUG(DAR_MSG_FULL, 
                  "<%d> %s:mkBuildNewList: Cassette installed.%c\n", ' ');
        }

        if ( !( MK_SENSOR_WORD & MK_MON_NOTALIGNED ) )
        {
            DEBUG(DAR_MSG_FULL, 
                  "<%d> %s:mkBuildNewList: Mask slot aligned.%c\n", ' ');
        }

        if ( !( MK_SENSOR_WORD & MK_MON_NOTOCCUPIED ) )
        {
            DEBUG(DAR_MSG_FULL, 
                  "<%d> %s:mkBuildNewList: Slot occupied.%c\n", ' ');
        } 
        ...
        {
            DEBUG(DAR_MSG_FULL, 
                  "<%d> %s:mkBuildNewList: index on home switch%c\n", ' ');
        ...
        {
            DEBUG(DAR_MSG_FULL, 
                  "<%d> %s:mkBuildNewList: index on low limit switch%c\n", ' '); 
                ...
                i++ )
            {
                ...
                tmp = &(mkIndexAreaAB[i]);
                DEBUG(DAR_MSG_FULL, 
                      "<%d> %s:mkBuildNewList: Adding task ==>%d\n", tmp->task );
                ellAdd( (ELLLIST *) &(pMkPriv->taskList), &(newTask->node) );
            }
        }
         ...
         */
         ...
         {
            DEBUG(DAR_MSG_ERROR, 
                  "<%d> %s:mkBuildNewList: Can't make safe index seq.%c\n", ' ');
        ...
        {
            DEBUG(DAR_MSG_ERROR, 
                  "<%d> %s:mkBuildNewList: Can't determine config. %c\n", ' ');
        ...
        {
            DEBUG(DAR_MSG_FULL, 
                  "<%d> %s:mkBuildNewList: Move out mask%c\n", ' ');
            for (i = 0; 
        ...
        {
            DEBUG(DAR_MSG_FULL,
                  "<%d> %s:mkBuildNewList: Moving into fp%c\n", ' ');
            for (i = 0; 
        ...
        if ( numConfig ==  MK_NOTHING && mode == DAR_MODE_MOVE )
        {
            ...
            DEBUG(DAR_MSG_FULL, 
                  "<%d> %s:mkBuildNewList: Nothing to do%c\n", ' ');
        }
        ...
        {
            DEBUG(DAR_MSG_ERROR, 
                  "<%d> %s:mkBuildNewList: Invalid assy configuration%c\n", ' ');
            SET_ERR_MSG("Mask assembly position invalid, help!");
            break;
        }
        ...
        if (MK_QUICK_UPDATE == TRUE)
        {
            ...
            DEBUG(DAR_MSG_WARNING, 
                  "<%d> %s:mkBuildNewList: WARNING! Performing QUICK UPDATE - this does NOT clear the existing barcode list!%c\n", ' ');
        }
        else
        {
            ...
            mkClearBarList( par );
            DEBUG(DAR_MSG_LOG, 
                  "<%d> %s:mkBuildNewList: FULL UPDATE - old list cleared.%c\n", ' ');
        }
        ...
        DEBUG(DAR_MSG_FULL, "<%d> %s:mkBuildNewList: adding mkInit task list %c\n", ' ');
        MK_CHECK_NULL( newTask = malloc( sizeof ( MK_LIST ) ), 
                       MK_MALLOC_ERROR );
        ...
        DEBUG(DAR_MSG_FULL, "<%d> %s:mkBuildNewList: adding mkTest task list %c\n", ' ');
        MK_CHECK_NULL( newTask = malloc( sizeof ( MK_LIST ) ), 
                       MK_MALLOC_ERROR );
...
)
{
    ...
    DEBUG(DAR_MSG_FULL, "<%d> %s:mkCheckAttributes: entry, sim=%d\n", 
    ...
    {
        DEBUG(DAR_MSG_ERROR, 
              "<%d> %s:mkCheckAttributes: track mode is not valid for mask assembly %c\n",  ' ' );
    ...
    {
        DEBUG(DAR_MSG_ERROR, 
              "<%d> %s:mkCheckAttributes: assembly is busy %c\n",  ' ' );
        status = MK_INVALID;
        ...
        par->mode == DAR_MODE_MOVE )
    {
        ...
        {
            DEBUG(DAR_MSG_ERROR,
                  "<%d> %s:mkCheckAttributes:invalid data types%c\n",' ' );
            status = MK_INVALID;
        ...
        else if (MK_MASK_IFU_LOC == MK_CMD_INBEAM)
        {
            ...
            {
                DEBUG(DAR_MSG_ERROR,"<%d> %s:mkCheckAttributes: Target mask ID out of range, ID=%d\n",
                      idTarget );
            ...
            {
                DEBUG(DAR_MSG_ERROR,
                      "<%d> %s:mkCheckAttributes:No barcodes%c\n",' ');
            ...
            else
            {
                for ( barCount = 0; barCount < MK_MAX_SLOTS; barCount++ )
                {
                    ...
                    {
                        DEBUG(DAR_MSG_MAX, 
                              "<%d> %s:mkCheckAttributes: checking: %ld\n",
                    ...
                    {
                        DEBUG(DAR_MSG_MAX, 
                              "<%d> %s:mkCheckAttributes: found at: %d\n",
                              barCount );
                        semTake (pMkPriv->mutexSem, WAIT_FOREVER);
        ...
        else if (MK_MASK_IFU_LOC != MK_CMD_OUTBEAM) 
        {
            ...
            DEBUG(DAR_MSG_ERROR,
                  "<%d> %s:mkCheckAttributes: invalid location%c\n", ' ');
                  ...
                  par->mode == DAR_MODE_PARK )
    {
        ...
        {
            DEBUG(DAR_MSG_ERROR, 
                  "<%d> %s:mkCheckAttributes: invalid park position data type%c\n",' ');
        ...
        {
            DEBUG(DAR_MSG_ERROR, 
                  "<%d> %s:mkCheckAttributes:bad park position%c\n",' ');
            status = MK_BAD_PARK_POSITION;
    ...
    if (par->dir == DAR_DIR_PRESET || par->dir == DAR_DIR_START)
    {
        ...
        {
            DEBUG(DAR_MSG_ERROR,
                  "<%d> %s:mkCheckAttributes:invalid data type, attr D/E%c\n",' ' );
            status = MK_INVALID;
        ...
        {
            DEBUG(DAR_MSG_ERROR,
                  "<%d> %s:mkCheckAttributes:attribute D out of range (normal extractor velocity)%c\n",
                  ' ' );
            status = MK_INVALID;
        ...
        {
            DEBUG(DAR_MSG_ERROR,
                  "<%d> %s:mkCheckAttributes:attribute E out of range (extractor indexing velocity)%c\n",
                  ' ' );
            status = MK_INVALID;
...
)
{
    ...
    DEBUG(DAR_MSG_FULL, "<%d> %s:mkCheckBarcodeId: entry%c\n",  ' ');
    ...
    {
        DEBUG(DAR_MSG_ERROR,"<%d> %s:mkCheckBarcodeId: Barcode Reader failure? Returned string=%s\n",
              idString );
    ...
    {
        DEBUG(DAR_MSG_FULL, 
              "<%d> %s:mkCheckBarcodeId: Read the barcode id.%c\n",' ');
        ...
        if ( *end != '\0' )
        {
            DEBUG(DAR_MSG_ERROR,"<%d> %s:mkCheckBarcodeId: Invalid barcode - not an integer, string=%s\n",
                  idString );
            status = MK_INVALID_BARCODE;
        }
        ...
        {
            DEBUG(DAR_MSG_ERROR,"<%d> %s:mkCheckBarcodeId: Barcode ID out of range, ID=%d\n", value );
            status = MK_INVALID_BARCODE;
        }
    }
    ...
    if ( pMkPriv->currentCmd == DAR_MODE_MOVE )
    {
        ...
        {
            DEBUG(DAR_MSG_ERROR,"<%d> %s:mkCheckBarcodeId: Barcode ID could not be verified (ID=%d), but move into focalplane anyway\n", value );
        ...
        if (status != MK_WRONG_BARCODE)
        {
            ...
            DEBUG(DAR_MSG_FULL,
                  "<%d> %s:mkCheckBarcodeId: Valid ID=%s\n", idString );
            semTake (pMkPriv->mutexSem, WAIT_FOREVER);
    ...
    else if ( pMkPriv->currentCmd == DAR_MODE_UPDATE )
    {
        ...
        if (status != DAR_S_SUCCESS)
        {
            ...
            value = MK_NO_BARCODE;
            DEBUG(DAR_MSG_WARNING,"<%d> %s:mkCheckBarcodeId: Flagging current mask as unidentifiable, node=%d\n", pMkPriv->barNodeNum-1 );
        ...
        else if ( pMkPriv->barNodeNum > 1 )
        {
            for (i=0; i < pMkPriv->barNodeNum-2; i++)
            {
                ...
                {
                    DEBUG(DAR_MSG_ERROR,
                        "<%d> %s:mkCheckBarcodeId: Duplicate%c\n",' ');
        ...
        if (status != MK_DUPLICATE_BARCODE)
        {
            ...
            {
                DEBUG(DAR_MSG_FULL,
                     "<%d> %s:mkCheckBarcodeId: Valid ID=%d\n",
                     pMkPriv->barCodeList[pMkPriv->barNodeNum-1].barCodeNum );
            }
            else 
            {
                DEBUG(DAR_MSG_FULL,
                     "<%d> %s:mkCheckBarcodeId: Dummy ID=%d\n",
                     pMkPriv->barCodeList[pMkPriv->barNodeNum-1].barCodeNum );
            }
        }
    }
    ...
    {
        DEBUG(DAR_MSG_ERROR,"<%d> %s:mkCheckBarcodeId: Should not be reading barcodes in this command mode:%d\n", pMkPriv->currentCmd );
...
)
{
    ...
    DEBUG(DAR_MSG_FULL, "<%d> %s:mkCheckCassette: entry%c\n",  ' ');
    ...
    if ( assSimulateLevel(par) != DAR_SIM_NONE )
    {
        ...
        {
            DEBUG(DAR_MSG_MAX, 
                  "<%d> %s:mkCheckCassette:Setting barcode read flag%c\n",' ');
            readTheBarCode = TRUE;
        }
    }
    ...
    else if (  ( MK_SENSOR_WORD & MK_MON_NOTINSTALLED ) )
    {
        ...
        {
            DEBUG(DAR_MSG_ERROR, 
                  "<%d> %s:mkCheckCassette: Cassette disappeared!%c\n",' ');
            status = MK_CAS_ERROR;
        ...
        {
            DEBUG(DAR_MSG_MAX, 
                  "<%d> %s:mkCheckCassette: Last cas empty, done%c\n",' ');
    ...
    else
    {
        ...
        {
            DEBUG(DAR_MSG_ERROR, 
                  "<%d> %s:mkCheckCassette: Not safe to remove mask%c\n",' ');
        ...
        else
        {
            ...
            {    
                goToNextSlot = TRUE;
                DEBUG(DAR_MSG_MAX, 
                      "<%d> %s:mkCheckCassette: Slot empty%c\n", ' ');
            }
        }
...
)
{
    ...
    DEBUG(DAR_MSG_FULL, "<%d> %s:mkCheckFocalPlane: entry%c\n", ' ');
    ...
    {
        DEBUG(DAR_MSG_MAX, 
              "<%d> %s:mkCheckFocalPlane: simulating, return%c\n", ' ');
        return( status );
    }
    ...
    {
        DEBUG(DAR_MSG_FULL, 
              "<%d> %s:mkCheckFocalPlane:Something in focal plane%c\n",' ');
        for ( i = 0; 
    ...
    {
        DEBUG(DAR_MSG_FULL, 
              "<%d> %s:mkCheckFocalPlane: Nothing in focal plane%c\n",' ');
        for ( i = 0; 
...
)
{
    ...
    DEBUG(DAR_MSG_FULL, "<%d> %s:mkClearBarList: entry, sim=%d\n", 
            assSimulateLevel(par) );
...
)
{
    ...
    DEBUG(DAR_MSG_FULL, "<%d> %s:mkCommandDevice: entry%c\n", ' ' );
    ...
    {
        DEBUG(DAR_MSG_MAX, 
              "<%d> %s:mkCommandDevice: Putting MOD, %d\n", mode );
    ...
    if (velLink->type != CONSTANT)
    {
        ...
        DEBUG(DAR_MSG_MAX, 
              "<%d> %s:mkCommandDevice: Putting VEL, %f\n", vel );
    ...
    {
        DEBUG(DAR_MSG_MAX, 
              "<%d> %s:mkCommandDevice: Putting DIR, %d\n", dir );
    ...
    DEBUG(DAR_MSG_MAX, 
          "<%d> %s:mkCommandDevice: Got response: %d\n", returnedAck );
    ...
    if (returnedAck == DAR_DEV_VAL_ACCEPT )
    {
        ...
        {
            DEBUG(DAR_MSG_WARNING, 
                  "<%d> %s:mkCommandDevice:failed to start watchdog timer - status:%d\n", status);
            status = DAR_S_SUCCESS;
        }
    }
    ...
    {
      DEBUG(DAR_MSG_ERROR, "<%d> %s:mkCommandDevice: %s failed\n", 
               deviceInfo->name);
...
)
{
    ...
    DEBUG(DAR_MSG_MAX, "<%d> %s:mkDoTask: entry%c\n", ' ');
    ...
    DEBUG(DAR_MSG_MAX, 
          "<%d> %s:mkDoTask: taskList size =%d\n", 
          ellCount( &(pMkPriv->taskList) ) );
    DEBUG(DAR_MSG_MAX, 
          "<%d> %s:mkDoTask: firsttask = %d\n", 
          taskItem->task );
    ...
    {
        DEBUG(DAR_MSG_WARNING, 
              "<%d> %s:mkDoTask:failed to stop watchdog timer - status:%d\n", status);
        status = DAR_S_SUCCESS;
    }
    ...
    while ( moreTasks && status == DAR_S_SUCCESS )
    {
            ...
            switch(taskItem->task )
        {
            ...
            case MK_INDEX_REL:         
                DEBUG(DAR_MSG_FULL, 
                  "<%d> %s:mkDoTask:** Index locking  mechanism%c\n", ' ');
            ...
            case MK_REL_TO_RELEASE:
                DEBUG(DAR_MSG_FULL, 
                      "<%d> %s:mkDoTask:** Release locking pin%c\n", ' ');
                status = mkCommandDevice (par, 
                                          &(pMkPriv->relDevice), 
                                          DAR_MODE_MOVE, 
                                          MK_NAME_POS_REL,
            ...
            case MK_REL_TO_LOCK:
                DEBUG(DAR_MSG_FULL, 
                      "<%d> %s:mkDoTask: Insert locking pin%c\n", ' ');
                status = mkCommandDevice (par, 
                                          &(pMkPriv->relDevice), 
                                          DAR_MODE_MOVE, 
                                          MK_NAME_POS_LOCK, 
            ...
            case MK_INDEX_EXT:
                DEBUG(DAR_MSG_FULL, 
                      "<%d> %s:mkDoTask:** Index Extractor%c\n", ' ' );
                status = mkCommandDevice (par, 
                                          &(pMkPriv->extDevice), 
                                          DAR_MODE_INDEX, 
                                          MK_NAME_POS_INDEX,
            ...
            case MK_EXT_TO_I4:
                DEBUG(DAR_MSG_FULL,
                      "<%d> %s:mkDoTask:** Pre-index ext to I4%c\n", ' ');
                status = mkCommandDevice (par, 
                                          &(pMkPriv->extDevice), 
                                          DAR_MODE_MOVE, 
                                          MK_NAME_POS_I4,
            ...
            case MK_EXT_TO_I4_GRP:
                DEBUG(DAR_MSG_FULL, 
                      "<%d> %s:mkDoTask:** Pre-index ext to grip%c\n", ' ');
                status = mkCommandDevice (par, 
                                          &(pMkPriv->extDevice), 
                                          DAR_MODE_MOVE, 
                                          MK_NAME_POS_I4_GRP,
            ...
            case MK_EXT_TO_I4_REL:
                DEBUG(DAR_MSG_FULL, 
                      "<%d> %s:mkDoTask:** Pre-index ext to rel%c\n", ' ');
                status = mkCommandDevice (par, 
                                          &(pMkPriv->extDevice), 
                                          DAR_MODE_MOVE, 
                                          MK_NAME_POS_I4_REL,
            ...
            case MK_EXT_TO_REL:
                DEBUG(DAR_MSG_FULL, 
                      "<%d> %s:mkDoTask:** Move extr release%c\n", ' ');
                status = mkCommandDevice (par, 
                                          &(pMkPriv->extDevice), 
                                          DAR_MODE_MOVE, 
                                          MK_NAME_POS_REL,
            ...
            case MK_EXT_TO_GRP:
                DEBUG(DAR_MSG_FULL, 
                      "<%d> %s:mkDoTask:** Move extr gripper%c\n", ' ');
                status = mkCommandDevice (par, 
                                          &(pMkPriv->extDevice), 
                                          DAR_MODE_MOVE, 
                                          MK_NAME_POS_GRP,
            ...
            case MK_EXT_TO_CLEAR:
                DEBUG(DAR_MSG_FULL, 
                      "<%d> %s:mkDoTask:** Move extr clear%c\n", ' ' );
                status = mkCommandDevice (par, 
                                          &(pMkPriv->extDevice), 
                                          DAR_MODE_MOVE, 
                                          MK_NAME_POS_CLEAR,
            ...
            case MK_EXT_TO_MASK:
                DEBUG(DAR_MSG_FULL, 
                      "<%d> %s:mkDoTask:** Move extr to mask posn%c\n", ' ');
                status = mkCommandDevice (par, 
                                          &(pMkPriv->extDevice), 
                                          DAR_MODE_MOVE, 
                                          MK_NAME_POS_MASK,
            ...
            case MK_EXT_TO_IFU:
                DEBUG(DAR_MSG_FULL, 
                      "<%d> %s:mkDoTask:** Move extr to ifu posn%c\n", ' ');
                status = mkCommandDevice (par, 
                                          &(pMkPriv->extDevice), 
                                          DAR_MODE_MOVE, 
                                          MK_NAME_POS_IFU,
            ...
            case MK_GRP_TO_RELEASE:
                DEBUG(DAR_MSG_FULL, 
                      "<%d> %s:mkDoTask:** Release gripper%c\n", ' ');
                status = mkCommandDevice (par, 
                                          &(pMkPriv->grpDevice), 
                                          DAR_MODE_MOVE, 
                                          MK_NAME_POS_REL,
            ...
            case MK_CAS_TO_C1S1:
                DEBUG(DAR_MSG_FULL, 
                      "<%d> %s:mkDoTask:** Move cas to first slot%c\n", ' ' );
                status = mkCommandDevice (par, 
                                          &(pMkPriv->casDevice), 
                                          DAR_MODE_MOVE, 
                                          "c1s1",
            ...
            case MK_CK_FOCAL_PLANE:
                DEBUG(DAR_MSG_FULL, 
                      "<%d> %s:mkDoTask:** Check focal plane%c\n", ' ');
                ...
                {
                    DEBUG(DAR_MSG_ERROR, "<%d> %s:mkDoTask: mkCheckFocalPlane() failed%c\n", ' ');
                    break;
                }
                ...
                {
                    DEBUG(DAR_MSG_FULL, "<%d> %s:mkDoTask: last task%c\n", ' ');
                    status = MK_LAST_TASK;
                    break;
                }
                ...
                else
                {
                    ...
                    {
                        DEBUG(DAR_MSG_ERROR, 
                              "<%d> %s:mkDoTask:task item for MK_CK_FOCAL_PLANE invalid:%d\n",
                              ellCount( &(pMkPriv->taskList) ) );
            ...
            case MK_CHECK_CASSETTE:
                ...
                {
                    DEBUG(DAR_MSG_ERROR, 
                          "<%d> %s:mkDoTask: mkCheckCassette() failed%c\n", ' ' );
                    break;
                }
                         ...
                         (MK_LIST *) ellFirst( &(pMkPriv->taskList) )) == NULL)
                {
                    ...
                    DEBUG(DAR_MSG_MAX, 
                          "<%d> %s:mkDoTask: replace check cassette%c\n", ' ');
                ...
                else
                {
                    ...
                    {
                        DEBUG(DAR_MSG_ERROR, 
                              "<%d> %s:mkDoTask: task item for MK_CHECK_CASSETTE invalid:%d\n", 
                              ellCount( &(pMkPriv->taskList) ) );
            ...
            case MK_ASKFOR_BARCODE_ID:
                DEBUG(DAR_MSG_FULL, 
                      "<%d> %s:mkDoTask:** Ask for barcode id.%c\n", ' ');
                    ...
                    DAR_S_SUCCESS )
                {
                    ...
                    DEBUG(DAR_MSG_ERROR, 
                          "<%d> %s:mkDoTask:Barcode trigger failed%c\n",' ');
                ...
                {
                    DEBUG(DAR_MSG_WARNING, 
                          "<%d> %s:mkDoTask:MK_ASK_FOR_BARCODE_ID: failed to start watchdog timer - status:%d\n", status);
                    status = DAR_S_SUCCESS;
                }
            ...
            case MK_INIT_ALL:
                DEBUG(DAR_MSG_FULL, 
                      "<%d> %s:mkDoTask:** Initialize all devices%c\n", ' ');
                status = mkCommandDevice (par, 
                                          &(pMkPriv->extDevice), 
                                          DAR_MODE_INIT, 
                                          "",
            ...
            case MK_TEST_ALL:         
                DEBUG(DAR_MSG_FULL, 
                      "<%d> %s:mkDoTask:** Test all devices%c\n", ' ');
                status = mkCommandDevice (par, 
                                          &(pMkPriv->extDevice), 
                                          DAR_MODE_TEST, 
                                          "",
            ...
            case MK_TURN_ON_OPTO:
                DEBUG(DAR_MSG_MAX, 
                 "<%d> %s:mkDoTask:** Turn on opto sensors if necessary.%c\n",' ');
                       ...
                       par->mode != DAR_MODE_INIT )
                {
                    ...
                    DEBUG(DAR_MSG_FULL, 
                         "<%d> %s:mkDoTask:** turning opto sensors ON %c\n", ' ' );
                                ...
                                DAR_S_SUCCESS )
                    {
                        status = MK_NO_POWER;
                        DEBUG(DAR_MSG_ERROR, 
                         "<%d> %s:mkDoTask:** Sensor power control failed%c\n",' ');
                    ...
                    DEBUG(DAR_MSG_MAX, 
                             "<%d> %s:mkDoTask:** opto sensors delay=%d secs\n", 
                             MK_SENSOR_DELAY );
                    ...
                    {
                        DEBUG(DAR_MSG_WARNING, 
                            "<%d> %s:mkDoTask:MK_TURN_ON_OPTO:ON failed to start watchdog timer - status:%d\n", status);
                        status = DAR_S_SUCCESS;
                    }
                }
                else 
                {
                    ...
                    DEBUG(DAR_MSG_FULL, 
                       "<%d> %s:mkDoTask:** opto sensors not required %c\n", ' ' );
                    assStartTimer( par, &status, 0 );
                    if (status != DAR_S_SUCCESS)
                    {
                        DEBUG(DAR_MSG_WARNING, 
                            "<%d> %s:mkDoTask:MK_TURN_ON_OPTO:OFF failed to start watchdog timer - status:%d\n", status);
                        status = DAR_S_SUCCESS;
                    }
                }
...
)
{
    ...
    DEBUG(DAR_MSG_FULL, "<%d> %s:mkEmptyList: entry%c\n", ' ');
    ...
    {
        DEBUG(DAR_MSG_MAX, 
              "<%d> %s:mkEmptyList: taskList is empty%c\n",' ' );
    ...
    {
        DEBUG(DAR_MSG_MAX, 
            "<%d> %s:mkEmptyList: did not empty=%d\n",
            ellCount( &(pMkPriv->taskList) ) );
    }
    }
...
)
{
    ...
    DEBUG(DAR_MSG_FULL, "<%d> %s:mkIndexMode: entry%c\n", ' ');
    ...
    if ( ( status = mkBuildList( par, DAR_MODE_INDEX ) ) != DAR_S_SUCCESS )
    {
        ...
        DEBUG(DAR_MSG_ERROR, 
                  "<%d> %s:mkIndexMode: mkbuildList failed%c\n", ' ');
        mkTerminateTasks (par, status);
    }
    ...
    {
        DEBUG(DAR_MSG_ERROR, 
              "<%d> %s:mkIndexMode: mkDoTask failed%c\n", ' ');
        mkTerminateTasks (par, status);
    }
...
)
{
    ...
    DEBUG(DAR_MSG_FULL, "<%d> %s:mkInitDeviceSupport: entry%c\n", ' ');
    ...
    {
        DEBUG(DAR_MSG_FATAL,
              "<%d> %s:mkInitDeviceSupport: malloc failed.%c\n", ' ');
    ...
    {
        DEBUG(DAR_MSG_FATAL, 
              "<%d> %s:mkInitDeviceSupport: mutex failed.%c\n", ' ' );
        status = MK_MALLOC_ERROR;
    ...
    {
        DEBUG(DAR_MSG_FATAL, 
              "<%d> %s:mkInitDeviceSupport:One or more devices not connected.%c\n",' ');
    ...
    {
        DEBUG(DAR_MSG_FATAL, 
              "<%d> %s:mkInitDeviceSupport: Barcode reader not connected.%c\n",' ' );
        status = MK_BAD_DEV_CONNECT;
    ...
    DEBUG(DAR_MSG_MAX, "<%d> %s:mkInitDeviceSupport: finished,status=%ld\n",  
        status );
...
)
{
    ...
    DEBUG(DAR_MSG_FULL, "<%d> %s:mkInitMode: entry%c\n", ' ');
    ...
    if ( ellCount( &(pMkPriv->taskList) ) != 0 )
    {
        DEBUG(DAR_MSG_MAX, "<%d> %s:mkInitMode: empty taskList =%d\n", 
              ellCount( &(pMkPriv->taskList) ) );
        mkEmptyList( par, TRUE );
    }
    ...
    {
        DEBUG(DAR_MSG_ERROR, 
              "<%d> %s:mkInitMode: failed to read lookup table%c\n", ' ' );
        mkTerminateTasks (par, status);
        return( status );
    }
    ...
    {
        DEBUG(DAR_MSG_ERROR, 
              "<%d> %s:mkInitMode: mkbuildList failed%c\n", ' ');
        mkTerminateTasks (par, status);
    }
    ...
    else if ( ( status = mkDoTask( par )) != DAR_S_SUCCESS )
    {
        mkTerminateTasks (par, status);
        DEBUG(DAR_MSG_ERROR, 
              "<%d> %s:mkInitMode: mkDoTask failed%c\n", ' ');
    }
...
)
{
    ...
    DEBUG(DAR_MSG_FULL, "<%d> %s:mkMoveConfig: entry%c\n", ' ');
    ...
    *numConfig = 0;
    ...
    {
        DEBUG(DAR_MSG_FULL, 
              "<%d> %s:mkMoveConfig: mask or IFU is in FP%c\n", ' ');
        ...
        {
            DEBUG(DAR_MSG_FULL, 
                  "<%d> %s:mkMoveConfig: mode MOVE with target IN%c\n",' ');
            ...
            {
                DEBUG(DAR_MSG_FULL, 
                "<%d> %s:mkMoveConfig: correct mask already there%c\n", ' ');
            ...
            {
                DEBUG(DAR_MSG_FULL, 
                      "<%d> %s:mkMoveConfig: wrong mask in%c\n",' ');
        ...
        {
            DEBUG(DAR_MSG_FULL, 
                  "<%d> %s:mkMoveConfig: mask is in, want it out%c\n", ' ');
            *numConfig = MK_MASK_OUT_FP;
        }
    } 
    ...
    {
        DEBUG(DAR_MSG_FULL, 
              "<%d> %s:mkMoveConfig: No mask in FP%c\n", ' ');
        ...
        {
            DEBUG(DAR_MSG_FULL, 
                  "<%d> %s:mkMoveConfig: no mask in, one is needed%c\n", ' ');
            *numConfig = MK_NEW_FP_IN;
        }
        ...
        {
            DEBUG(DAR_MSG_FULL, 
                  "<%d> %s:mkMoveConfig: No mask in, none needed%c\n", ' ');
                 *numConfig = MK_NOTHING;
        }
    }
...
)
{
    ...
    DEBUG(DAR_MSG_MAX, "<%d> %s:mkMoveMode: entry%c\n", ' ');
        ...
        pMkPriv->currentLoc == MK_CMD_OUTBEAM ) )
    {
        ...
        DEBUG(DAR_MSG_FULL, 
              "<%d> %s:mkMoveMode: clearing currentCmd%c\n", ' ');
    }
    ...
    if ( par->mode == DAR_MODE_MOVE || par->mode == DAR_MODE_PARK )
    {
        ...
        {
            DEBUG(DAR_MSG_ERROR, 
                  "<%d> %s:mkMoveMode: mkbuildList failed%c\n", ' ');
        }
    }
    else
    {
        ...
        DEBUG(DAR_MSG_ERROR,
              "<%d> %s:mkMoveMode: invalid move mode%c\n", ' ');
    }
    ...
    else 
    {
        ...
        {   
            DEBUG(DAR_MSG_ERROR, 
              "<%d> %s:mkMoveMode: mkDoTask failed%c\n", ' ');
            mkTerminateTasks (par, status);
        }
    }
...
)
{
    ...
    DEBUG(DAR_MSG_MAX, "<%d> %s:mkProcessFault: entry%c\n", ' ');
...
)
{
    ...
    DEBUG(DAR_MSG_MAX, "<%d> %s:mkReadBarList: entry%c\n", ' ');
    ...
    {   
        ...
        if (( fp = fopen( scratchBuf, "r")) == NULL )
        {
            ...
            {
                DEBUG(DAR_MSG_ERROR, 
                      "<%d> %s:mkReadBarList: File: %s does not exist and cannot be created\n",
                      scratchBuf);
            ...
            {
                DEBUG(DAR_MSG_WARNING, 
                      "<%d> %s:mkReadBarList: Warning! %s did not exist - created file stub\n",
                      scratchBuf);
            }
        }
        ...
        mkClearBarList( par );
        DEBUG(DAR_MSG_MIN, "<%d> %s:mkReadBarList: Old list cleared.%c\n", ' ');
        ...
        {
            DEBUG(DAR_MSG_MAX, "<%d> %s:mkReadBarList:got a line: %s\n", 
            scratchBuf ); 
            ...
            else        
            {
                ...
                {
                    DEBUG(DAR_MSG_ERROR, 
                          "<%d> %s:mkReadBarList: Bad line in mask lookup table:%s\n", 
                          scratchBuf );
                ...
                {
                    DEBUG(DAR_MSG_ERROR, 
                          "<%d> %s:mkReadBarList: Slot or cassette number out of range in LUT line:%s\n",
                          scratchBuf);
                    SET_ERR_MSG("Mask barcode file corrupted, use UPDATE");
                    status = MK_FILE_FORMAT_ERROR;
                    break;
        ...
        {
            DEBUG(DAR_MSG_ERROR, 
                  "<%d> %s:mkReadBarList: Failed to read barcode list:%c\n",' ');
            if ( pMkPriv->barListEmpty == FALSE )
            {
                mkClearBarList( par );
                DEBUG(DAR_MSG_MIN, "<%d> %s:mkReadBarList: Partial list cleared.%c\n", ' ');
            }
        }
        else
        {
    ...
    {
        DEBUG(DAR_MSG_ERROR, 
              "<%d> %s:mkReadBarList: Mask barcode directory/filename not specified%c\n",  ' ');
...
)
{
    ...
    DEBUG(DAR_MSG_MAX, "<%d> %s:mkStopDirective: entry%c\n", ' ');
    ...
    {
        DEBUG(DAR_MSG_LOG, 
              "<%d> %s:mkStopDirective:Will abort command when safe%c\n",' ');
        semTake (pMkPriv->mutexSem, WAIT_FOREVER);
    ...
    {
        DEBUG(DAR_MSG_MIN, 
              "<%d> %s:mkStopDirective:no cmd active%c\n",' ');
        mkTerminateTasks (par, status);
    }
...
)
{
    ...
    DEBUG(DAR_MSG_MAX, "<%d> %s:mkTaskFinished: entry%c\n", ' ');
    ...
    {
        DEBUG(DAR_MSG_ERROR, 
              "<%d> %s:mkTaskFinished: task list is already empty!%c\n", ' ');
    ...
    DEBUG(DAR_MSG_MAX, 
          "<%d> %s:mkTaskFinished: current task = %d\n", 
          currentTask->task);
    ...
    DEBUG(DAR_MSG_MAX, 
          "<%d> %s:mkTaskFinished: expect= %d\n", numCompleted );

    numCompleted = pMkPriv->extFinished + 
                   pMkPriv->relFinished +
                   pMkPriv->grpFinished + 
                   pMkPriv->casFinished;
    DEBUG(DAR_MSG_MAX, "<%d> %s:mkTaskFinished: completed= %d\n", 
          numCompleted );
    ...
    { 
        ...
        DEBUG(DAR_MSG_FULL, 
              "<%d> %s:mkTaskFinished: Task finished: %d\n", 
              currentTask->task);
        assStopTimer( par, &status );
        if (status != DAR_S_SUCCESS)
        {
            DEBUG(DAR_MSG_WARNING, 
                  "<%d> %s:mkTaskFinished:failed to stop watchdog timer - status:%d\n", status);
            status = DAR_S_SUCCESS;
        }
        ...
        {
            DEBUG(DAR_MSG_ERROR, 
                "<%d> %s:mkTaskFinished, post check failed%c\n", ' ');
        }
        ...
        {
            DEBUG(DAR_MSG_FULL, 
                  "<%d> %s:mkTaskFinished, all tasks done%c\n", ' ');
            semTake (pMkPriv->mutexSem, WAIT_FOREVER);
            ...
            if ( pMkPriv->currentCmd == DAR_MODE_UPDATE )
            {
                ...
                {
                    DEBUG(DAR_MSG_WARNING, 
                          "<%d> %s:mkTaskFinished:barcode reader failed, will still write partial list (status: %d)\n", 
                          pMkPriv->status);
                }
                ...
                if (pMkPriv->barListEmpty == FALSE)
                {
                    if ( (lutStatus = mkWriteBarList( par )) != DAR_S_SUCCESS )
                    {
                        ...
                        DEBUG(DAR_MSG_ERROR, 
                              "<%d> %s:mkTaskFinished:write fail status: %d\n", 
                              lutStatus);
                    }
                }
        ...
        else
        {
            ...
            {
                DEBUG(DAR_MSG_ERROR, 
                      "<%d> %s:mkTaskFinished: mkDoTask failed%c\n", ' ');
            }
        }
    }
    ...
    {
        DEBUG(DAR_MSG_FULL, "<%d> %s:mkTaskFinished: Task not done%c\n", ' ');
    }
...
)
{
    ...
    DEBUG(DAR_MSG_MAX, "<%d> %s:mkTaskPostCheck: entry%c\n", ' ');
    ...
    {
        DEBUG(DAR_MSG_ERROR, "<%d> %s:mkTaskPostCheck: taskList empty =%d\n", 
              ellCount( &(pMkPriv->taskList) ) );
        status = MK_LAST_TASK;
    ...
    if ( assSimulateLevel(par ) != DAR_SIM_NONE )
    {
        ...
        else if (taskPerformed->task == MK_REL_TO_RELEASE)
        {
            ...
            MONITOR_VALB;
            DEBUG(DAR_MSG_FULL, 
                  "<%d> %s:mkTaskPostCheck: set MASK_LOC to: %ld\n", 
                  MK_EXT_MASK_POS);
        } 
                 ...
                 (taskPerformed->task == MK_CAS_TO_C1S1) )
        {
            ...
            {
                DEBUG(DAR_MSG_FULL, 
                      "<%d> %s:mkTaskPostCheck: Stopping simulated motion after task:%d\n",taskPerformed->task);
        ...
        DEBUG(DAR_MSG_MAX,
              "<%d> %s:mkTaskPostCheck: simulating, return%c\n", ' ');
    ...
    switch( taskPerformed->task )
    {
        ...
        case MK_INDEX_REL:
            ...
            {
                DEBUG(DAR_MSG_ERROR, 
                      "<%d> %s:mkTaskPostCheck: Locking pin not in%c\n",' ');
        ...
        case MK_REL_TO_RELEASE:
            ...
            {
                DEBUG(DAR_MSG_ERROR, 
                      "<%d> %s:mkTaskPostCheck: Locking pin not out%c\n", ' ');
            ...
            if (status == DAR_S_SUCCESS)
            {
                ...
                MONITOR_VALB;
                DEBUG(DAR_MSG_FULL, 
                      "<%d> %s:mkTaskPostCheck:Mask pos unknown%c\n", ' ');
            }
            break;
        ...
        case MK_REL_TO_LOCK:
            ...
            if ((status == DAR_S_SUCCESS) && (MK_SENSOR_WORD & MK_MON_NOTINFP))
            {
                ...
                MONITOR_VALB;
                DEBUG(DAR_MSG_FULL, 
                      "<%d> %s:mkTaskPostCheck: Mask is in cassette%c\n",' ');
            }
            break;
        ...
        case MK_EXT_TO_I4:
            ...
            {
                DEBUG(DAR_MSG_ERROR, 
                      "<%d> %s:mkTaskPostCheck: extractor not in limit%c\n", 
                ' ');
        ...
        case MK_EXT_TO_GRP:
            ...
            {
                DEBUG(DAR_MSG_ERROR, 
                      "<%d> %s:mkTaskPostCheck: extractor not in grip position%c\n",' ');
            ...
            if ( MK_SENSOR_WORD & MK_MON_NOTOCCUPIED )
            {
                DEBUG(DAR_MSG_ERROR, "<%d> %s:mkTaskPostCheck: No mask in slot after MK_EXT_TO_I4_GRP or MK_EXT_TO_GRP%c\n",  ' ');
        ...
        case MK_EXT_TO_REL:
            ...
            {
                DEBUG(DAR_MSG_ERROR, 
                      "<%d> %s:mkTaskPostCheck: Extractor not in release position%c\n",' ');
        ...
        case MK_EXT_TO_MASK:
            ...
            {
                DEBUG(DAR_MSG_ERROR,
                      "<%d> %s:mkTaskPostCheck: Nothing in FP%c\n", ' ');
            ...
            else if ( !(MK_SENSOR_WORD & MK_MON_IFUNOTIN) )
            {
                ...
                {
                    DEBUG(DAR_MSG_ERROR, 
                          "<%d> %s:mkTaskPostCheck:Mask switch fault - cannot be both IFU and Mask%c\n",' ');
            ...
            {
                DEBUG(DAR_MSG_ERROR, 
                      "<%d> %s:mkTaskPostCheck:Mask switch fault - must be either IFU or mask%c\n", ' ');
                SET_ERR_MSG("Mask type switch failure");
                status = MK_TASK_FAILED;
            }
            ...
            if (status == DAR_S_SUCCESS)
            {
                ...
                MONITOR_VALB;
                DEBUG(DAR_MSG_FULL, 
                      "<%d> %s:mkTaskPostCheck: Mask in beam%c\n", ' ');
            }
            break;
        ...
        case MK_EXT_TO_IFU:
            ...
            {
                DEBUG(DAR_MSG_ERROR, 
                      "<%d> %s:mkTaskPostCheck: No IFU in FP%c\n", ' ');
            ...
            else if (status == DAR_S_SUCCESS)
            {
                ...
                MONITOR_VALB;
                DEBUG(DAR_MSG_FULL, 
                      "<%d> %s:mkTaskPostCheck: IFU in beam %c\n", ' ');
            }
            break;
        ...
        case MK_INDEX_GRP:
            ...
            {
                DEBUG(DAR_MSG_ERROR, 
                      "<%d> %s:mkTaskPostCheck: grip not released%c\n", ' ');
        ...
        case MK_GRP_TO_RELEASE:
            ...
            {
                DEBUG(DAR_MSG_FULL, 
                      "<%d> %s:mkTaskPostCheck: Stop current command after moving grip to release position%c\n", ' ');
                semTake (pMkPriv->mutexSem, WAIT_FOREVER);
                pMkPriv->stopRequested = FALSE;
                semGive (pMkPriv->mutexSem);
        ...
        case MK_GRP_TO_GRIP:
            ...
            {
                DEBUG(DAR_MSG_ERROR, 
                      "<%d> %s:mkTaskPostCheck: Grip not gripped%c\n", ' ');
            ...
            {
                DEBUG(DAR_MSG_FULL, 
                      "<%d> %s:mkTaskPostCheck:Stop current command after moving grip to grip position%c\n",' ');
                semTake (pMkPriv->mutexSem, WAIT_FOREVER);
                pMkPriv->stopRequested = FALSE;
                semGive (pMkPriv->mutexSem);
        ...
        case MK_CAS_TO_PARK:
            DEBUG(DAR_MSG_FULL,
                  "<%d> %s:mkTaskPostCheck: setting casParked%c\n", ' ');
            semTake (pMkPriv->mutexSem, WAIT_FOREVER);
        ...
        case MK_CAS_TO_SLOT:
            ...
            {
                DEBUG(DAR_MSG_ERROR, 
                      "<%d> %s:mkTaskPostCheck: Slot not aligned%c\n", ' ');
        ...
        case MK_CAS_TO_C1S1:
            ...
            {
                DEBUG(DAR_MSG_FULL, 
                      "<%d> %s:mkTaskPostCheck: Stop current command after moving cassette to C1S1 position%c\n", ' ');
                semTake (pMkPriv->mutexSem, WAIT_FOREVER);
                pMkPriv->stopRequested = FALSE;
                semGive (pMkPriv->mutexSem);
        ...
        default:
            DEBUG(DAR_MSG_ERROR, "<%d> %s:mkTaskPostCheck: Invalid task=%d\n", 
                  taskPerformed->task );
...
)
{
    ...
    DEBUG(DAR_MSG_MAX, "<%d> %s:mkTaskPreCheck: entry%c\n", ' ');
    ...
    {
        DEBUG(DAR_MSG_ERROR, 
              "<%d> %s:mkTaskPreCheck: taskList empty =%d\n", 
              ellCount( &(pMkPriv->taskList) ) );
        status = MK_LAST_TASK;
    ...
    if ( assSimulateLevel(par ) != DAR_SIM_NONE )
    {
        if (taskPerformed->task == MK_EXT_TO_REL)
        {
            ...
            MONITOR_VALB;
            DEBUG(DAR_MSG_FULL, 
                  "<%d> %s:mkTaskPreCheck: Faking mask motion%c\n",' ');
        }

        DEBUG(DAR_MSG_MAX, 
              "<%d> %s:mkTaskPreCheck: simulating, return%c\n", ' ');
        return( status );
    }
    ...
    switch( taskPerformed->task )
    {
        ...
        case MK_REL_TO_RELEASE:
            ...
            {
                DEBUG(DAR_MSG_ERROR, 
                      "<%d> %s:mkTaskPreCheck: Extractor not in release position%c\n",' ');
        ...
        case MK_EXT_TO_REL:
            ...
            {
                DEBUG(DAR_MSG_ERROR, 
                      "<%d> %s:mkTaskPreCheck: Slot not aligned %c\n",' ');
                SET_ERR_MSG("Cassette slot not aligned");
                status = MK_TASK_FAILED;
            }
            ...
            if (!(MK_EXT_POSITION & MK_EXT_AT_GRIPPER))
            {
                ...
                {
                    DEBUG(DAR_MSG_ERROR, 
                          "<%d> %s:mkTaskPreCheck: Locking pin in%c\n",' ');
            ...
            if (status == DAR_S_SUCCESS)
            {
                ...
                MONITOR_VALB;
                DEBUG(DAR_MSG_FULL, 
                      "<%d> %s:mkTaskPreCheck: mask loc unknown%c\n", ' ');
            }
        ...
        case MK_EXT_TO_GRP:
            ...
            if (!(MK_EXT_POSITION & MK_EXT_AT_RELEASE))
            {
                ...
                {
                    DEBUG(DAR_MSG_ERROR, 
                          "<%d> %s:mkTaskPreCheck:grip not released%c\n", ' ');
                ...
                {
                    DEBUG(DAR_MSG_ERROR,
                          "<%d> %s:mkTaskPreCheck: No mask in slot before MK_EXT_TO_GRP%c\n", ' ');
        ...
        case MK_EXT_TO_IFU:
            ...
            {
                DEBUG(DAR_MSG_ERROR, 
                      "<%d> %s:mkTaskPreCheck: Not an IFU!%c\n", ' ');
        ...
        case MK_EXT_TO_MASK:
            ...
            {
                DEBUG(DAR_MSG_ERROR, 
                      "<%d> %s:mkTaskPreCheck: locking pin not removed%c\n", ' ');
            ...
            {
                DEBUG(DAR_MSG_ERROR, 
                      "<%d> %s:mkTaskPreCheck: not gripping mask%c\n", ' ');
        ...
        case MK_EXT_TO_CLEAR:
            ...
            {
                DEBUG(DAR_MSG_ERROR,
                      "<%d> %s:mkTaskPreCheck: locking pin not inserted%c\n", ' ');
            ...
            {
                DEBUG(DAR_MSG_ERROR, 
                      "<%d> %s:mkTaskPreCheck: Grip is not released%c\n", ' ');
        ...
        case MK_GRP_TO_RELEASE:
            ...
            {
                DEBUG(DAR_MSG_ERROR, 
                      "<%d> %s:mkTaskPreCheck: Extractor not at grip pos%c\n",  ' ');
                SET_ERR_MSG("Extractor not at grip position");
                status = MK_TASK_FAILED;
            }
        ...
        case MK_GRP_TO_GRIP:
            ...
            {
                DEBUG(DAR_MSG_ERROR, 
                      "<%d> %s:mkTaskPreCheck: No mask in the slot%c\n", ' ');
    ...
    case MK_CAS_TO_SLOT:
            ...
            DEBUG(DAR_MSG_FULL, 
                  "<%d> %s:mkTaskPreCheck: casParked=%d\n", 
                   pMkPriv->casParked);
            ...
            {
                DEBUG(DAR_MSG_ERROR, 
                      "<%d> %s:mkTaskPreCheck:Cassette not fully inserted%c\n", ' ');
            ...
            {
                DEBUG(DAR_MSG_ERROR, 
                      "<%d> %s:mkTaskPreCheck:Extractor not clear of cassette%c\n", ' ');
        ...
        default: 
            ...
            DEBUG(DAR_MSG_ERROR, "<%d> %s:mkTaskPreCheck: Invalid task=%d\n", 
                  taskPerformed->task );
...
)
{
    ...
    DEBUG(DAR_MSG_FULL, "<%d> %s:mkTerminateTasks: entry%c\n", ' ');
    ...
    {
        DEBUG(DAR_MSG_WARNING, 
                  "<%d> %s:mkTerminateTasks:failed to stop watchdog timer - status:%d\n", timerStatus);
    }
    semTake (pMkPriv->mutexSem, WAIT_FOREVER);
     ...
     */
     ...
     DEBUG(DAR_MSG_FULL, 
            "<%d> %s:mkTerminateTasks: turning opto sensors OFF %c\n",  ' ' );
             ...
             DAR_S_SUCCESS )
     {
         ...
         DEBUG(DAR_MSG_ERROR, 
                  "<%d> %s:mkTerminateTasks:Sensor power control failed%c\n",' ');
...
)
{
    ...
    DEBUG(DAR_MSG_MAX, "<%d> %s:mkTestMode: entry%c\n", ' ');
    ...
    {
        DEBUG(DAR_MSG_ERROR, 
              "<%d> %s:mkTestMode: Task list not empty.%c\n", ' ' );
    ...
    {
        DEBUG(DAR_MSG_ERROR, 
              "<%d> %s:mkTestMode: mkbuildList failed%c\n", ' ');
        mkTerminateTasks (par, status);
        return status;
    }
    else if ( ( status = mkDoTask( par )) != DAR_S_SUCCESS )
    {
        DEBUG(DAR_MSG_ERROR, 
              "<%d> %s:mkTestMode: mkDoTask failed%c\n", ' ');
        mkTerminateTasks (par, status);
        return status;
    }
...
)
{
    ...
    DEBUG(DAR_MSG_MAX, "<%d> %s:mkTestSensors: entry%c\n", ' ');
    ...
    DEBUG(DAR_MSG_MIN, "<%d> %s:mkTestSensors:Extractor word is:0x%x\n", MK_EXT_POSITION );
    if ( (MK_EXT_POSITION & MK_EXT_AT_RELEASE ) )
    {
        DEBUG(DAR_MSG_MIN, "<%d> %s:mkTestSensors:Ext in release position%c\n", ' ');
    }
    if ( (MK_EXT_POSITION & MK_EXT_AT_GRIPPER ) )
    {
        DEBUG(DAR_MSG_MIN, "<%d> %s:mkTestSensors:Ext in grip position%c\n", ' ');
    }
    if ( (MK_EXT_POSITION & MK_EXT_BTW_R_G ) )
    {
        DEBUG(DAR_MSG_MIN, "<%d> %s:mkTestSensors:Ext between Release & Grip%c\n", ' ');
    }
    if ( (MK_EXT_POSITION & MK_EXT_IN_LVDT ) )
    {
        DEBUG(DAR_MSG_MIN, "<%d> %s:mkTestSensors:Ext in LVDT area%c\n", ' ');
    }
    if ( (MK_EXT_POSITION & MK_EXT_ATPAST_REL ) )
    {
        DEBUG(DAR_MSG_MIN, "<%d> %s:mkTestSensors:Ext at or Past Release position%c\n", ' ');
    }
    if ( (MK_EXT_POSITION & MK_EXT_BEFR_GRP ) )
    {
        DEBUG(DAR_MSG_MIN, "<%d> %s:mkTestSensors:Ext before grip position%c\n", ' ');
    }
    ...
    {
        DEBUG(DAR_MSG_ERROR,
              "<%d> %s:mkTestSensors:Ext reads before and after grip%c\n",' ');
    ...
    {
        DEBUG(DAR_MSG_ERROR, 
              "<%d> %s:mkTestSensors:Ext reads before and after release%c\n", ' ');
        SET_ERR_MSG ("Invalid extractor position");
        status = MK_BAD_SENSORS;
    }
    ...
    {
        DEBUG(DAR_MSG_ERROR, 
              "<%d> %s:mkTestSensors:Ext reads in and out of LVDT%c\n", ' ');
        SET_ERR_MSG ("Invalid extractor position");
        status = MK_BAD_SENSORS;
    }
    ...
    {
        DEBUG(DAR_MSG_ERROR, 
              "<%d> %s:mkTestSensors: Ext LVDT position conflict%c\n", ' ');
        SET_ERR_MSG ("Invalid extractor position");
        status = MK_BAD_SENSORS;
    }
    ...
    DEBUG(DAR_MSG_MIN, "<%d> %s:mkTestSensors:Sensor word is:0x%x\n", MK_SENSOR_WORD );
    ...
    {
        DEBUG(DAR_MSG_MIN, "<%d> %s:mkTestSensors:Release in lock position%c\n", ' ');
    }
    else if ( !( MK_SENSOR_WORD & MK_MON_RELNOTREL ) &&
               ( MK_SENSOR_WORD & MK_MON_RELNOTLCK ) )
    {
        DEBUG(DAR_MSG_MIN, "<%d> %s:mkTestSensors:Release in release position%c\n", ' ');
    }
    else if ( ( MK_SENSOR_WORD & MK_MON_RELNOTLCK ) &&
              ( MK_SENSOR_WORD & MK_MON_RELNOTREL ) )
    {
        DEBUG(DAR_MSG_ERROR, 
              "<%d> %s:mkTestSensors: Release between positions%c\n", ' ');
    ...
    {
        DEBUG(DAR_MSG_ERROR, 
              "<%d> %s:mkTestSensors: Mask release switch failure%c\n", ' ');
    ...
    {
        DEBUG(DAR_MSG_MIN, "<%d> %s:mkTestSensors:Gripper in release position%c\n", ' ');
    }
    else if ( !( MK_SENSOR_WORD & MK_MON_GRPNOTGRP ) &&
               ( MK_SENSOR_WORD & MK_MON_GRPNOTREL ) )
    {
        DEBUG(DAR_MSG_MIN, "<%d> %s:mkTestSensors:Gripper in grip position%c\n", ' ');
    }
    else if ( ( MK_SENSOR_WORD & MK_MON_GRPNOTGRP ) &&
              ( MK_SENSOR_WORD & MK_MON_GRPNOTREL ) )
    {
        DEBUG(DAR_MSG_ERROR, 
              "<%d> %s:mkTestSensors: Gripper between positions%c\n", ' ');
    ...
    {
        DEBUG(DAR_MSG_ERROR, 
              "<%d> %s:mkTestSensors: Mask gripper switch failure%c\n", ' ');
    ...
    if ( !( MK_SENSOR_WORD & MK_MON_NOTINSTALLED ) )
    {
        DEBUG(DAR_MSG_MIN, "<%d> %s:mkTestSensors:Cassette installed.%c\n", ' ');
    }
    if ( !( MK_SENSOR_WORD & MK_MON_NOTALIGNED ) )
    {
        DEBUG(DAR_MSG_MIN, "<%d> %s:mkTestSensors:Mask slot aligned .%c\n", ' ');
    }
    if ( !( MK_SENSOR_WORD & MK_MON_NOTOCCUPIED ) )
    {
        DEBUG(DAR_MSG_MIN, "<%d> %s:mkTestSensors:Slot occupied.%c\n", ' ');
    }
    if ( !( MK_SENSOR_WORD & MK_MON_NOTALIGNED ) &&
          ( MK_SENSOR_WORD & MK_MON_NOTINSTALLED ) )
    {
        DEBUG(DAR_MSG_ERROR, 
              "<%d> %s:mkTestSensors:Slot aligned without a cassette?%c\n",' ');
    ...
    {
        DEBUG(DAR_MSG_ERROR, 
              "<%d> %s:mkTestSensors: slot occupied yet not aligned?%c\n",' ');
    ...
    if ( !( MK_SENSOR_WORD & MK_MON_NOTINFP ) &&
         !( MK_SENSOR_WORD & MK_MON_MASKNOTIN ) &&
          ( MK_SENSOR_WORD & MK_MON_IFUNOTIN ) )
    {
        DEBUG(DAR_MSG_MIN, "<%d> %s:mkTestSensors:Mask In focal plane.%c\n", ' ');
    }
              ...
              !( MK_SENSOR_WORD & MK_MON_IFUNOTIN ) &&
               ( MK_SENSOR_WORD & MK_MON_MASKNOTIN ) )
    {
        DEBUG(DAR_MSG_MIN, "<%d> %s:mkTestSensors:IFU In focal plane.%c\n", ' ');
    }
              ...
              ( MK_SENSOR_WORD & MK_MON_IFUNOTIN ) )
    {
        DEBUG(DAR_MSG_MIN, "<%d> %s:mkTestSensors:Focal plane is empty.%c\n", ' ');
    }

    else 
    {
        DEBUG(DAR_MSG_ERROR, 
              "<%d> %s:mkTestSensors:Conflicting FP switch info%c\n",' ');
...
)
{
    ...
    DEBUG(DAR_MSG_MAX, "<%d> %s:mkTrackMode: nothing to track%c\n", ' ');
...
)
{
    ...
    DEBUG(DAR_MSG_MAX, "<%d> %s:mkUpdateMode: entry%c\n", ' ');
    ...
    {   
        ...
        sprintf (scratchBuf, "%s/%s", par->tdir, par->tfil);
        DEBUG(DAR_MSG_MIN, 
              "<%d> %s:mkUpdateMode:opening file: %s\n", par->tfil);

        if ( (fp = fopen(scratchBuf, "w")) == NULL)
        {
            DEBUG(DAR_MSG_ERROR, 
                  "<%d> %s:mkUpdateMode: cannot open file %s\n", scratchBuf);
            SET_ERR_MSG("Cannot open mask barcode file");
            status = MK_FILE_OPEN_ERROR;
        }
        else
        {
            ...
            DEBUG(DAR_MSG_FULL, 
                  "<%d> %s:mkUpdateMode:Closing file: %s\n", par->tfil);
            fclose (fp);
        }
    }
    else
    {
        DEBUG(DAR_MSG_ERROR, 
              "<%d> %s:mkUpdateMode: Mask barcode directory/filename not specified%c\n",  ' ');
    ...
    {
        DEBUG(DAR_MSG_ERROR, 
              "<%d> %s:mkUpdateMode: mkbuildList failed%c\n", ' ');
        mkTerminateTasks (par, status);
        return status;
    }
    ...
    {
        DEBUG(DAR_MSG_ERROR, 
              "<%d> %s:mkUpdateMode: mkDoTask failed%c\n", ' ');
        mkTerminateTasks (par, status);
    }
...
)                                                                    
{
    ...
    DEBUG(DAR_MSG_FULL, "<%d> %s:mkWriteBarList: entry%c\n", ' ');
    ...
    {   
        ...
        sprintf (scratchBuf, "%s/%s", par->tdir, par->tfil);
        DEBUG(DAR_MSG_MIN, 
              "<%d> %s:mkWriteBarList:Opening file: %s\n", par->tfil);

        if ( (fp = fopen(scratchBuf, "w")) == NULL)
        {
            DEBUG(DAR_MSG_ERROR, 
                  "<%d> %s:mkWriteBarList: File open failed%c\n", ' ');
            SET_ERR_MSG("Cannot open mask barcode file");
            status = MK_FILE_OPEN_ERROR;
            return ( status );
        }
        ...
        for ( barCount = 0; barCount < MK_MAX_SLOTS; barCount++ )
        {
            if ( pMkPriv->barCodeList[barCount].barCodeNum != MK_NO_ENTRY )
            {
                ...
                {
                     DEBUG(DAR_MSG_MAX, "<%d> %s:mkWriteBarList: Wrote line: %s", scratchBuf);
                }
            }
            if (status == MK_BAD_WRITE)
            {
                SET_ERR_MSG("Mask barcode file write failed");
                DEBUG(DAR_MSG_ERROR, "<%d> %s:mkWriteBarList: Write failed%c\n",' ');
                break;
            }
        ...
        DEBUG(DAR_MSG_FULL, 
              "<%d> %s:mkWriteBarList:Closing file: %s\n", par->tfil);
        fclose (fp);
    ...
    {
        DEBUG(DAR_MSG_ERROR, 
              "<%d> %s:mkWriteBarList: Mask barcode directory/filename not specified%c\n",  ' ');
        SET_ERR_MSG("Mask lookup table dir/file unspecified");
        status = MK_FILE_OPEN_ERROR;
    }
