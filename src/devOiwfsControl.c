...
)
{
    ...
    DEBUG(DAR_MSG_FULL, "<%d> %s:oiBusyStateChange: entry%c\n", ' ');
    ...
    {
        DEBUG(DAR_MSG_MIN, 
              "<%d> %s:oiBusyStateChange: unexpected busy state change%c\n", ' ');
        return status;
    }
    ...
    if (assCommandTimedOut (par))
    {
        ...
        {
            DEBUG(DAR_MSG_ERROR, 
                  "<%d> %s:oiBusyStateChange: Base stage did not start in time%c\n", ' ');
        ...
        {
            DEBUG(DAR_MSG_ERROR, 
                  "<%d> %s:oiBusyStateChange: Pickoff stage did not start in time%c\n", ' ');
        ...
        {
            DEBUG(DAR_MSG_ERROR, 
                  "<%d> %s:oiBusyStateChange: Base stage did not stop in time%c\n", ' ');
        ...
        {
            DEBUG(DAR_MSG_ERROR, 
                  "<%d> %s:oiBusyStateChange: Pickoff stage did not stop in time%c\n", ' ');
        ...
        {
            DEBUG(DAR_MSG_ERROR, 
                  "<%d> %s:oiBusyStateChange: Base stage did not finish in time%c\n", ' ');
        ...
        {
            DEBUG(DAR_MSG_ERROR, 
                  "<%d> %s:oiBusyStateChange: Pickoff stage did not finish in time%c\n", ' ');
        ...
        {
            DEBUG(DAR_MSG_ERROR, 
                  "<%d> %s:oiBusyStateChange: Unexpected command timeout%c\n", ' ');
    ...
    if (baseBusyChange) switch (par->bus1)
    {
        ...
        case DAR_DEV_BUSY_ERROR:
            ...
            semGive (pDevPvt->mutexSem);
            DEBUG(DAR_MSG_ERROR, 
                  "<%d> %s:oiBusyStateChange: Base stage error%c\n", ' ');
            oiCancelCommand (par);
            break;
    }
    ...
    if (pickoffBusyChange) switch (par->bus2)
    {
        ...
        case DAR_DEV_BUSY_ERROR:
            ...
            semGive (pDevPvt->mutexSem);
            DEBUG(DAR_MSG_ERROR, 
                  "<%d> %s:oiBusyStateChange: Pickoff stage error%c\n", ' ');
            oiCancelCommand (par);
            break;
    }
...

    {
        ...
        {
            DEBUG(DAR_MSG_FULL, 
                  "<%d> %s:oiBusyStateChange:calculating pre-index move%c\n",' ');
        ...
        {
            DEBUG(DAR_MSG_FULL, 
                  "<%d> %s:oiBusyStateChange: index on home switch%c\n",' ');
        ...
        if (pDevPvt->inLimMove)
        {
            ...
            DEBUG(DAR_MSG_FULL, 
                  "<%d> %s:oiBusyStateChange: in-limit move done, recover targets%c\n",' ');
            /*
            ...
            {
                DEBUG(DAR_MSG_ERROR, 
                      "<%d> %s:oiBusyStateChange: Angles calculated cannot be reached!%c\n",' ');
        ...
        par->bus2 != DAR_DEV_BUSY_BUSY)
    {
        ...
        {
            DEBUG(DAR_MSG_ERROR, 
                  "<%d> %s:oiBusyStateChange: Base stage failed command%c\n", ' ');
        ...
        {
            DEBUG(DAR_MSG_ERROR, 
                  "<%d> %s:oiBusyStateChange: Pickoff stage failed command%c\n", ' ');
...
)
{
    ...
    DEBUG(DAR_MSG_FULL, "<%d> %s:oiCancelCommand: entry%c\n", ' ');
    ...
    {
        DEBUG(DAR_MSG_FULL, 
              "<%d> %s:oiCancelCommand: stopping base stage %c\n", ' ');
    ...
    {
        DEBUG(DAR_MSG_FULL, 
              "<%d> %s:oiCancelCommand: stopping pickoff stage %c\n", ' ');
...
)
{
    ...
    DEBUG(DAR_MSG_FULL, "<%d> %s:oiCheckAttributes: entry%c\n", ' ');
    ...
    {
        DEBUG(DAR_MSG_ERROR, 
              "<%d> %s:oiCheckAttributes: UPDATE mode invalid for OIWFS%c\n",' ');
        ...
        par->mode == DAR_MODE_PARK)
    {
        ...
        {
            DEBUG(DAR_MSG_ERROR, 
                  "<%d> %s:oiCheckAttributes: X target out of range%c\n", ' ');
            sprintf (buffer, 
        ...
        {
            DEBUG(DAR_MSG_ERROR, 
                  "<%d> %s:oiCheckAttributes: Y target out of range%c\n", ' ');
            sprintf (buffer,
        ...
        {
            DEBUG(DAR_MSG_ERROR, 
                  "<%d> %s:oiCheckAttributes: Angles calculated cannot be reached!%c\n",' ');
    ...
    if (par->mode != DAR_MODE_TRACK)
    {
        ...
        {
            DEBUG(DAR_MSG_ERROR, 
                  "<%d> %s:oiCheckAttributes: Base command rejected%c\n",' ');
        ...
        {
            DEBUG(DAR_MSG_ERROR, 
                  "<%d> %s:oiCheckAttributes: Pickoff command rejected%c\n",' ');
...
)
{
    ...
    DEBUG(DAR_MSG_FULL, "<%d> %s:oiConvertTarget: entry%c\n", ' ');
    ...
    {
        DEBUG(DAR_MSG_ERROR, "<%d> %s:oiConvertTarget: gmOiwfsCalculateProbeAngles failed%c\n", ' ');
        return status;
    }
...
)
{
    ...
    DEBUG(DAR_MSG_FULL, "<%d> %s:oiExecuteCommand: entry%c\n", ' ');
    ...
    {
        DEBUG(DAR_MSG_ERROR, 
              "<%d> %s:oiExecuteCommand: Base Stage rejected command%c\n",' ');
    ...
    {
        DEBUG(DAR_MSG_ERROR, 
              "<%d> %s:oiExecuteCommand: Pickoff Stage rejected command%c\n",' ');
    ...
    {
        DEBUG(DAR_MSG_MAX, 
              "<%d> %s:oiExecuteCommand: Re-calculate motion timeout%c\n",' ');
        assStopTimer (par, &status);
        assStartTimer (par, &status, 
                       (((runTimeout=OI_MAX_RUN_TIMEOUT-36*OI_VELOCITY)<0 ) ?
                        OI_MIN_RUN_TIMEOUT : runTimeout ) ) ;
        DEBUG(DAR_MSG_MAX, 
              "<%d> %s:oiExecuteCommand: Motion timeout re-calculated:%d\n",runTimeout);
    }
...
)
{
    ...
    DEBUG(DAR_MSG_FULL, "<%d> %s:oiIndexMode: entry%c\n", ' ');
    ...
    {
        DEBUG(DAR_MSG_FULL,
              "<%d> %s:oiIndexMode: simulating pre-index move%c\n", ' ');
    ...
    else
    {
        ...
        {
            DEBUG(DAR_MSG_FULL, 
                  "<%d> %s:oiIndexMode: using single pre-index move%c\n",' ');
        }
          ...
          /*
            ...
            if (OI_Y_ABS < ((OI_Y_LO_LIM + OI_Y_HI_LIM)/2.0))
            {
                ...
                if ((OI_PRESET_ANGLE - OI_BAS_ABS) > 0)
                {
                    ...
                    DEBUG(DAR_MSG_FULL, 
                          "<%d> %s:oiIndexMode:pko avoids Y low lim%c\n",' ');
                ...
                else if ((OI_PRESET_ANGLE - OI_PKO_ABS) > 0)
                {
                    ...
                    DEBUG(DAR_MSG_FULL, 
                          "<%d> %s:oiIndexMode:base avoids Y low lim%c\n",' ');
                }
                ...
                {
                    DEBUG(DAR_MSG_FULL, 
                          "<%d> %s:oiIndexMode:won't hit Y lo lim%c\n",' ');
                }
            }
            ...
            else
            {
                ...
                if ((OI_PRESET_ANGLE - OI_BAS_ABS) < 0)
                {
                    ...
                    baseTarget = (OI_BAS_INCR + 1.0);
                    DEBUG(DAR_MSG_FULL, 
                          "<%d> %s:oiIndexMode:pko avoids Y hi lim%c\n",' ');
                }
                ...
                else if ((OI_PRESET_ANGLE - OI_PKO_ABS) < 0)
                {
                    ...
                    pickoffTarget = (OI_PKO_INCR + 1.0);
                    DEBUG(DAR_MSG_FULL, 
                          "<%d> %s:oiIndexMode:base avoids Y hi lim%c\n",' ');
                }
                ...
                {
                    DEBUG(DAR_MSG_FULL, 
                          "<%d> %s:oiIndexMode:won't hit Y hi lim%c\n",' ');
                }
            }
        }
...
)
{
    ...
    DEBUG(DAR_MSG_FULL, "<%d> %s:oiInitDeviceSupport: entry%c\n", ' ');
    ...
    {
        DEBUG(DAR_MSG_FATAL, 
              "<%d> %s:oiInitDeviceSupport: mutex failed.%c\n", ' ' );
        status = DAR_E_MALLOC;
...
{
    DEBUG(DAR_MSG_FULL, "<%d> %s:oiInitMode: entry%c\n", ' ');
...
)
{
    ...
    DEBUG(DAR_MSG_FULL, "<%d> %s:oiMoveMode: entry%c\n", ' ');
...
)
{
    ...
    DEBUG(DAR_MSG_MAX, "<%d> %s:oiProcessFault: entry%c\n", ' ');
    ...
    {
        DEBUG(DAR_MSG_LOG, "<%d> %s:oiProcessFault: Cancelling current command%c\n", ' ');
        semTake (pDevPvt->mutexSem, WAIT_FOREVER);
...
{
    DEBUG(DAR_MSG_FULL, "<%d> %s:oiTestMode: entry%c\n", ' ');
...
{
    DEBUG(DAR_MSG_FULL, "<%d> %s:oiTrackMode: entry%c\n", ' ');
...
)
{
    ...
    DEBUG(DAR_MSG_FULL, "<%d> %s:oiUpdateMode: doing nothing%c\n", ' ');
